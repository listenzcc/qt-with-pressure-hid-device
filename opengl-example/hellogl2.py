...
class GLWidget(QOpenGLWidget, QOpenGLFunctions):
    ...
    def initializeGL(self):
        ...
        self.program.addShaderFromSourceCode(
            QOpenGLShader.Vertex, self._vertex_shader)
        self.program.addShaderFromSourceCode(
            QOpenGLShader.Fragment, self._fragment_shader)
        ...
        with QOpenGLVertexArrayObject.Binder(self.vao):
            ...
            self._logo_vbo.allocate(
                self.logo.const_data(), self.logo.count() * float_size)
            ...
            self.program.setUniformValue(
                self._light_pos_loc, QVector3D(0, 0, 70))
    ...
    def setup_vertex_attribs(self):
        ...
        f.glVertexAttribPointer(0, 3, int(GL.GL_FLOAT), int(
            GL.GL_FALSE), 6 * float_size, null)
        f.glVertexAttribPointer(1, 3, int(GL.GL_FLOAT), int(
            GL.GL_FALSE), 6 * float_size, pointer)
    ...
    def paintGL(self):
        ...
        with QOpenGLVertexArrayObject.Binder(self.vao):
            ...
            self.program.setUniformValue(
                self._mv_matrix_loc, self.camera * self.world)
            normal_matrix = self.world.normalMatrix()
            self.program.setUniformValue(
                self._normal_matrix_loc, normal_matrix)
